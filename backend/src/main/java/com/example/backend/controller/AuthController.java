...
public class AuthController {
    ...
    public ResponseEntity<?> login(@Valid @RequestBody LoginRequest loginRequest) {
        ...
        try {
            ...
            logger.error("로그인 처리 중 내서버 오류 발생 | 이메일: '{}'", loginRequest.getEmail(), e);
        ...
        try {
            if (userService.isSocialUser(email)) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                        .body("소셜 로그인 계정은 비밀번호를 재설정할 수 없습니다.");
            }
            userService.requestPasswordReset(email);
            return ResponseEntity.ok("비밀번호 재설정 링크 전송");
        } catch (EntityNotFoundException e) {
            logger.info("존재하지 않는 이메일에 대한 비밀번호 재설정 요청: {}", email);
            userService.requestPasswordReset(email);
            return ResponseEntity.ok("입력하신 이메일로 비밀번호 재설정 안내 메일이 발송되었습니다.");
